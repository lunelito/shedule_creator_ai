...
export async function GET(
  ...
  try {
    ...
    if (skill.length === 0) return NextResponse.json({ error: 'Skill not found' }, { status: 404 });
...
export async function PUT(
  ...
  try {
    ...
    const updatedSkill = await db.update(skills)
      ...
      .returning();
    if (updatedSkill.length === 0) return NextResponse.json({ error: 'Skill not found' }, { status: 404 });
...
export async function DELETE(
  ...
  try {
    const deletedSkill = await db.delete(skills)
      .where(eq(skills.id, parseInt(params.id)))
      .returning();
    if (deletedSkill.length === 0) return NextResponse.json({ error: 'Skill not found' }, { status: 404 });
