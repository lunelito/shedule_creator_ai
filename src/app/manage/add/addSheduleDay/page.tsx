...
export default function AddScheduleDay() {
  ...
  const { formatedData, getRemainingWeeklyHours, CheckIfCantWork, parseData } =
    useScheduleLogic({ dataSingleScheduleDay: dataSingleScheduleDayFetched });
  ...
  const getDataFromLocalHost = () => {
    ...
    const initialShifts: EmployeeShift[] = employeesTabFetched.map((emp) => {
      ...
      console.log("halo")
      const scheduleSwap = scheduleSwapRequestsFetched.some((el) => {
        ...
        return (
          isWaiting &&
          ((isReceiver && isReceiveDate) ||
            (isRequester && isRequestDate) ||
            (isRequester && isReceiveDate) ||
            (isReceiver && isRequestDate))
        );
      });
  ...
  useEffect(() => {
    if (employeesTabFetched && dataThreeMonthScheduleDayAllFetched && scheduleSwapRequestsFetched) {
      getDataFromLocalHost();
    }
  }, [employeesTabFetched, dataThreeMonthScheduleDayAllFetched,scheduleSwapRequestsFetched]);
