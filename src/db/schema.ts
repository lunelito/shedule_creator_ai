...
export const teams = pgTable('teams', { // grupy pracowników np kuchnia, zarząd
  id: serial('id').primaryKey(),
...
export const locations = pgTable('locations', { //  różne adresy/oddziały typu Restauracja Warszawa Centrum - ul. Marszałkowska 1, Restauracja Kraków Stare Miasto - ul. Grodzka 5
  id: serial('id').primaryKey(),
  organization_id: integer('organization_id').references(() => organizations.id).notNull(),
  name: varchar('name', { length: 160 }).notNull(),
...
export const roles = pgEnum('roles', ['admin', 'manager', 'supervisor', 'employee'])
...
export const overtime_policies = pgTable('overtime_policies', {
  ...
  daily_threshold_hours: numeric('daily_threshold_hours', { precision: 4, scale: 2 }),
  weekly_threshold_hours: numeric('weekly_threshold_hours', { precision: 4, scale: 2 }),
  overtime_multiplier: numeric('overtime_multiplier', { precision: 3, scale: 2 }).default('1.5'),
